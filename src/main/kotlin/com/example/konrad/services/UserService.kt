...
class UserService(
    ...
    fun getAllPatientList(userToken: String): ResponseEntity<*> {
        ...
        return ResponseEntity.ok(ResponseModel(success = true, body = patientList.map { PatientDetailsObject.toModel(it) }))
    }
    ...
    fun getPatientById(id: String): ResponseEntity<*> {
        ...
        return if(response.isPresent) {
            ...
            ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ResponseModel(success = false, body = null, reason = "No patient found with this id"))
        }
    }
}